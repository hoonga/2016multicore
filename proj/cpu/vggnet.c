...
static void pooling_layer(float * inputs, float * outputs, int N, int D)
{
  ...
  size_t g[1] = {D};
  size_t l[1] = {16};
  clEnqueueNDRangeKernel(q, pool, 1, NULL, g, l, 0, NULL, NULL);
/*
  int i;
  for(i = 0; i < D; i++)
  {
...
static void convolution_layer(float * inputs, float * outputs, float * filters, float * biases, int N, int D1, int D2)
{
  ...
  size_t g[1] = {D2};
  size_t l[1] = {16};
  clEnqueueNDRangeKernel(q, conv, 1, NULL, g, l, 0, NULL, NULL);
...
void vggnet(float * images, float * network, int * labels, float * confidences, int num_images)
{
  ...
  cl_device_id did[1];
  CL_CHECK(clGetDeviceIDs(pid[0], CL_DEVICE_TYPE_CPU, 1, did, NULL));
